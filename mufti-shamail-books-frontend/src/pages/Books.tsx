...
import { ArrowRight, Loader2, Play, Pause, Filter } from "lucide-react";
import BookCard from "../components/books/BookCard";
import { useEffect, useState, useRef } from "react";
import { Book, getAllBooks, getBookLanguages, BookLanguage, LanguageOption } from "../apis/books.api";
...
const Books = () => {
	...
	const [selectedLanguage, setSelectedLanguage] = useState<BookLanguage | ''>('');
	...
	useEffect(() => {
		const fetchBooks = async () => {
			setIsLoading(true);
			try {
				const { data } = await getAllBooks(selectedLanguage || undefined);
				setBooks(data);
			} catch (error) {
				toast.error(
			...
			<section className="relative py-16 md:py-24 overflow-hidden">
				...
				>
					<svg width="60" height="60" viewBox="0 0 60 60" fill="none">
						<path d="M20,12 Q12,20 12,30 Q12,40 20,48 Q26,42 26,30 Q26,18 20,12" fill="#c3e5a5" opacity="0.7" />
						<path
				...
				<div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
						...
						>
							...
							>
								Welcome to <span className="text-[#c3e5a5] drop-shadow-lg">Mufti Shamail</span> Books
							</motion.h1>
							<p className="text-gray-300 text-lg md:text-xl mb-8 leading-relaxed">
								Discover authentic Islamic knowledge through carefully curated books that bridge classical scholarship
								with contemporary understanding. Explore our collection of spiritual guidance, jurisprudence, and
						...
						>
							...
							>
								...
								>
									<source
										src="https://res.cloudinary.com/dwoke3tu3/video/upload/v1755010039/IMG_0653_qrzpgu.mov"
										type="video/mp4"
									/>
								...
								<div className="absolute inset-0 flex items-center justify-center bg-black/30">
									<motion.button
										...
										whileTap={{ scale: 0.95 }}
									>
										{isVideoPlaying ? <Pause className="w-10 h-10" /> : <Play className="w-10 h-10 ml-1" />}
									</motion.button>
								</div>
							</motion.div>
						</motion.div>
					...
					>
						...
						<div className="flex justify-center items-center gap-4 mb-8">
							...
							<select
								...
								className="bg-[#24271b] text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#c3e5a5] transition-all border border-[#c3e5a5]/20"
							>
			...
			<section className="relative py-16 md:py-24">
				...
				>
					...
					<p className="text-gray-500 mt-4 text-lg">
						Stay tuned for upcoming releases that will further enrich your Islamic knowledge
					</p>
				</motion.div>
			</section>
		</main>
