...
export const getPublishedFatwahs = async (
  ...
  try {
    ...
    // Enhanced search functionality - search in question and answer fields
    if (search) {
...
export const getFatwahById = async (
  ...
  try {
    const fatwah = await Fatwah.findById(req.params.id).populate(
      "answeredBy",
      "name"
    );
...
export const submitQuestion = async (
  ...
  try {
    ...
    res.status(201).json({
      ...
      data: fatwah,
      message:
        "Your question has been submitted successfully. We will review and publish the answer soon.",
    });
  } catch (error) {
    next(error);
  }
