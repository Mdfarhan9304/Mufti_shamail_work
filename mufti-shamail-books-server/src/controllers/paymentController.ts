...
export const generatePaymentUrl = async (
  ...
  try {
    ...
    if (
      ...
      res.status(200).json({
        success: true,
        redirectUrl: response.data.data.instrumentResponse.redirectInfo.url, // Redirect URL for the user to complete payment
      });
    }
  } catch (error) {
    next(error);
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (existingOrder) {
        ...
        console.log(
          "Sending existing order response:",
      ...
      const orderData = {
        ...
        contactDetails: req.user
          ? {
              phone: req.user?.phone || "0000000000", // Default phone if not available
              email: req.user?.email || "",
              name: req.user?.name || "",
            }
          : {
              phone: guestInfo?.phone || "0000000000", // Default phone for guest
              email: guestInfo?.email || "hsfhh@email.com",
              name: guestInfo?.name || "fkmfnj",
            },
        shippingAddress,
        txnId: merchantTransactionId,
